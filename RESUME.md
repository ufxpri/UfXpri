...
`flask` `tensorflow` `REST API` `async` `jwt`
...
`nginx` `MSA` `REST API` `AWS` `tensorflow`
...
`python` `RTSP` `stream data` `tensorflow`
...
- cctv 스트리밍 데이터를 활용한 AI 분석 시스템 설계 및 개발
- 

#### 드론 AI 통합 관제 시스템 (2020.11 ~ 2022.03)
`python` `DDD` `mjpeg` `klv` `AWS` `gRPC`
...
`python` `gRPC` `docker` `pytorch` `On-Premise` `MSA` `ffmpeg`
...
`FastAPI` `rabbitmq` `docker` `docker-compose` `MSA`
